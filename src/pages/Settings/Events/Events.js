...
function onAfterDeleteRow(row, rowKeys) {
  ...
  for(var i in row){
    console.log(row[i]);
     var deleteRef = database.ref('events/'+row[i]);
     deleteRef.remove();
  }
}
//table function
...
class Events extends Component {
  ...
  gotData = (data) => {
    ...
    for (let i = 0; i < keys.length; i++) {
      const k = keys[i];
      newProducts.push({
        title: userdata[k].title, content: userdata[k].content, id: userdata[k].id, date: userdata[k].date,
        time: userdata[k].time,
      });
    }
    ...
    return (
      <div style={containerStyle} className="pt-card">
        ...
        >
          ...
          <TableHeaderColumn dataField='content' dataSort={true} onChange={this.handleChange} >Agenda</TableHeaderColumn>
          <TableHeaderColumn dataField='date' dataSort={true} onChange={this.handleChange} >Date</TableHeaderColumn>
          <TableHeaderColumn dataField='time' dataSort={true} onChange={this.handleChange} >Time</TableHeaderColumn>
